...
app.config["excel_uploads"] = r"C:\Users\suraj\Desktop\Final_Year_Project\Sappy_flask_app\static\exel"
app.config["get_result"] = r"C:\Users\suraj\Desktop\Final_Year_Project\Sappy_flask_app\static\exel\results"
    ...
    if request.method == "POST":
        if request.files:
            ...
            if (excel and allowed_file(excel.filename)) and (excel_PO and allowed_file(excel_PO.filename)):
                ...
                print(GRN_file)
                global url
                url = "C:\\Users\\suraj\\Desktop\\Final_Year_Project\\Sappy_flask_app\\static\\exel\\%s" % (GRN_file)
                global url_PO
                url_PO = "C:\\Users\\suraj\\Desktop\\Final_Year_Project\\Sappy_flask_app\\static\\exel\\%s" % (PO_file)
...
def build_csv_data(dataframe):
    csv_data = dataframe.to_excel(
        r"C:\Users\suraj\Desktop\Final_Year_Project\Sappy_flask_app\static\exel\results\results.xlsx", index=False, encoding='utf-8')
...
def consumption_preprocess(dataframe):
    ...
    # return dataframe
    Desc = dataframe[["Item No.", "Item Description", "Acp. Qty.",
                           "GRN Date", "Rate", "Buyer", "month", "day", "PO UOM", "Currency"]]
...
def avgconsumption(dataframe, val):
    if val == '5':
        ...
        df_yearly_consumption = pd.DataFrame({
            ...
            'Avg total consumption': items_group['Acp. Qty.'].mean(),
...
@app.route("/allConsumption", methods=['GET', 'POST'])
@login_required
def allConsumption():
    if True:
        global CSV
        ...
        if form.validate_on_submit() or request.method == 'POST':
            ...
            if form.duration.data == '5':
                ...
                df.index += 1
                return render_template('table.html',  tables=[df.to_html(classes='data', header="true")], the_title='Consumption', titles=df.columns.values, form=form)
        return render_template("allConsumption.html", form=form)
    else:
        flash(f'Please Upload files!', 'warning')
        return redirect('/home')
...
@app.route("/allFrequency", methods=['GET', 'POST'])
@login_required
def allFrequency():
    if True:
        global CSV
        ...
        if request.method == 'POST':
            ...
            flash(f'"Item Description": Item Description given in the GRN Report <br> "Qx_ordered_qty": Qty. ordered in that quarter/year <br> "max_freq_count" : The maximum frequency of order <br> "max_freq_quarter" : Quarter with maximum order frequency <br> "max_order_qty" : Maximum ordered qty. in a quarter <br> "max_order_quarter" : The quarter in which maximum qty. was ordered', 'secondary')
            return render_template('table.html',  tables=[df.to_html(classes='data', header="true")], the_title='Order frequency', titles=df.columns.values, form=form)
        return render_template("allFrequency.html", form=form)
    else:
        flash(f'Please Upload files!', 'warning')
        return redirect('/home')
